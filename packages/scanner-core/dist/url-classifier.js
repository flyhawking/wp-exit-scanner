...
exports.classifyPermalinks = classifyPermalinks;
function classifyPermalinks(hrefs) {
    ...
    for (const raw of hrefs) {
        ...
        if (p === '/' || /\/(feed|comments|author|tag|category)(\/|$)/.test(p))
            continue;
        if (/\.(html?|php|css|js|xml|txt|jpe?g|png|webp|svg|ico|pdf)$/i.test(p))
            continue;
        if (/\/\d{4}\/\d{1,2}(\/\d{1,2})?\//.test(u.pathname))
            samples.date++;
        else {
            const depth = p.split('/').filter(Boolean).length;
            const looksLikeSlug = /^[\p{L}\p{N}%._~-]+$/u.test(p.split('/').filter(Boolean).pop() || '');
            if (looksLikeSlug && depth <= 4)
                samples.postname++;
            else
                samples.other++;
        }
    ...
    if (total > 0) {
        if (samples.query / total > 0.5)
            structure = 'query';
        else if (samples.date / total > 0.3)
            structure = 'date-based';
        else if (samples.postname / total >= 0.4)
            structure = 'postname';
    }
