...
export interface RuleContext {
  builders: string[];
  plugins: string[];
  theme: { type: 'block' | 'classic' | 'unknown'; slug: string | null };
  hasAcf: boolean;
...
export function evaluate(ctx: RuleContext): ScoreResult {
  const findings: Finding[] = [];
  const flags: string[] = [];
  let score = 100;
  const penalize = (penalty: number) => {
    score -= penalty;
  };
  ...
  if (ctx.builders.length > 0) {
    ...
    add({
      ...
      detail:
        'Page-builder layouts are stored as shortcodes/DOM blobs and cannot be faithfully converted to clean templates in v1. ' +
  ...
  const cats = categorizePlugins(ctx.plugins);
  ...
  if (blockers.length) {
    add({
      ...
      detail:
        `We detected ${blockers.join(' and ')}. Static publishing would remove the core features your site runs on. ` +
  ...
  const light: Light = hardBlocked || score < 50 ? 'red' : score < 80 ? 'yellow' : 'green';

  const path: MigrationPath = flags.includes('ECOMMERCE') || flags.includes('MEMBERSHIP')
    ? 'not-suitable'
...
function summarize(light: Light, path: MigrationPath, ctx: RuleContext): string {
